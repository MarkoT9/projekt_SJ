<?php
// Zahrnutie pripojenia k databáze a triedy produktov
include_once 'classes/Db_connection.php';
include_once 'classes/Product.php';

// Overenie, či je ID produktu odoslané v URL (napr. edit-product.php?id=1)
if (isset($_GET['id'])) {
    $productId = $_GET['id'];
    ...
    // Načítanie údajov o produkte z databázy
    $productData = $product->readOne($productId);

    // Overenie, či produkt existuje
    if (!$productData) {
...
// Spracovanie odoslania formulára na aktualizáciu produktu
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $unitsSold = $_POST['units_sold'];  // Získanie predaných jednotiek

    // Zavolanie metódy na aktualizáciu
...
<script>
    $(function () {
        // Inicializácia jQuery UI datepicker pre pole expire_date
        $('#expire_date').datepicker({
            dateFormat: 'yy-mm-dd'  // Nastavenie formátu dátumu na yyyy-mm-dd
        });
    });
</script>
</body>
